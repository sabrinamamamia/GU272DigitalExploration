...
//Load JSON 
var json = d3.json('gu272-data.json', function (error, json) {
	if (error) return console.error(error);	

	filteredJson = []
	for (var id in json) {
		setChildren(json, id);
		if (json[id].children.length >= 2 && json[id].parent == null) {
			filteredJson.push(json[id]);
		}
	}

	root = d3.hierarchy(filteredJson[0]);
	root.x0 = height/2;
	root.y0 = 0;
	...
	function update(source) {
	  ...
	  nodes.forEach(function(d){ 
	    // d.y = d.depth * (width / (treeData.height + 2))
	    d.y = d.depth * 180;
	  });
